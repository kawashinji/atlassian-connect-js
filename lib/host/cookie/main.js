...
(function (define, AJS) {
    ...
    define("ac/cookie/main", [], function () {
        ...
        return {
            saveCookie: function saveCookie(addonKey, name, value, expires) {
                AJS.Cookie.save(prefixCookie(addonKey, name), value, expires);
            },
            readCookie: function readCookie(addonKey, name, callback) {
                var value = AJS.Cookie.read(prefixCookie(addonKey, name));
                if (typeof callback === "function") {
                    callback(value);
                }
            },
            eraseCookie: function eraseCookie(addonKey, name) {
